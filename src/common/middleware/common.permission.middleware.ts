...
class CommonPermissionMiddleware{
    ...
    permissionFlagRequired(requiredPermissionFlag: PermissionFlag){
        return ( req: express.Request, res: express.Response, next: express.NextFunction) => {
            try{ 
                ...
                userPermissionFlags && requiredPermissionFlag?  next(): res.status(403).send();
            }
            catch(err){
                log(err)
            }
    ...
    async onlySameUserAndAdminCanDoThisAction(req: express.Request, res: express.Response, next: express.NextFunction){
        const userPermissionFlags = parseInt(res.locals.jwt.permissionFlags);
